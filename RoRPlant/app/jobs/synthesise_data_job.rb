...
class SynthesiseDataJob < ApplicationJob
  ...
  def initialize(*args)
    super(*args)
    @system = SystemGraph.load_dependencies_graph(DEMO_OUTPUT_UUID);
    @risk_assessor = RiskAssessor.new(@system, DEMO_OUTPUT_UUID);
    @sim = nil
    @mlocs = {}
  ...
  def perform(*args)
    ...
    @sim = PlantSim.new(@system, @sim_start_time, @xfer_functions)
    ...
    while self.keep_running and simtime_s < (3600 * 12) do
      ...
      @sim.sim_step(SIM_TIME_PER_SECOND)
