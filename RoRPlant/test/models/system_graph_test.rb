...
class SystemGraphTest < ActiveSupport::TestCase
  ...
  def test_bfs_nodes_with_edgetest()
    self.setup_diamond();
    init_r = [];
    avoid_ac = lambda { |edge| 
      return "1-9-2" != edge.conn.uuid
    }
    res = @g.bfs_node_visitor(@a, init_r, multivisit: false, edge_filter: avoid_ac) { |e, n, r|
      r << "#{n.seg.uuid} via #{e ? e.conn.shortname : nil}."
    }
    assert res.size == 3
    assert res[2].starts_with?("1-4 ")
    assert res.all?(& lambda { |x|
      return x.index('1-3') == nil;
    });
