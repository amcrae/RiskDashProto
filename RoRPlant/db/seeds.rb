...
# The data can then be loaded with the bin/rails db:seed command (or created alongside the database with db:setup).
    ...
    SegmentConnection.new(from_segment_id:m1.id, to_segment_id:p1.id, shortname:'Driveshaft', segtype:'SHAFT').save()
    SegmentConnection.new(from_segment_id:m2.id, to_segment_id:p2.id, shortname:'Driveshaft', segtype:'SHAFT').save()
    
    SegmentConnection.new(from_segment_id:in1.id, to_segment_id:p1.id, shortname:'in', segtype:'PIPE').save()
    SegmentConnection.new(from_segment_id:p1.id, to_segment_id:out1.id, shortname:'out', segtype:'PIPE').save()
    
    SegmentConnection.new(from_segment_id:in1.id, to_segment_id:p2.id, shortname:'in', segtype:'PIPE').save()
    SegmentConnection.new(from_segment_id:p2.id, to_segment_id:out1.id, shortname:'out', segtype:'PIPE').save()
